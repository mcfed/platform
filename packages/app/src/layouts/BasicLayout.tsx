...
import { fetchReset } from '@mcfed/core/dist/middleware/fetching';
...
export function BasicLayout(props: any) {
  const { locale, dispatch, appReducer } = props;
  ...
  const breadcrumbRoutes = useBreadcrumbRoutes(activeRoutes, locale, pathname);
    ...
    return (
      <Menu.Item key={currentRoute.key || currentRoute.path}>
        <NavLink
          ...
          <span>{locale(currentRoute.name)}</span>
        </NavLink>
      </Menu.Item>
    );
  }
    ...
    <AppLayout className={styles['basic-layout']}>
      ...
      <AppLayout>
        <Header className={styles['basic-layout-header']}>
          <GlobalHeader user={user} onMenuSelect={handleMenuSelect}>
          </GlobalHeader>
        </Header>
        <AppBreadcrumbs
