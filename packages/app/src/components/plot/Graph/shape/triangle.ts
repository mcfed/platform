...
Shape.registerNode(
  'triangle',
  {
    ...
    drawShape(cfg: NodeConfig, group: IGroup): IShape {
      const { icon = {}, direction: defaultDirection } = (this as any).getOptions(cfg) as NodeConfig;
      ...
      const keyShape = group.addShape('path', {
        ...
        name: `${(this as any).type}-keyShape`,
        draggable: true,
      });

      const { width: w, height: h, show, offset=0 } = icon;
      if (show) {
        let iconW = -w! / 2;
        let iconH = -h! / 2;
    ...
    drawLinkPoints(cfg: NodeConfig, group: IGroup) {
      const { linkPoints = {}, direction: defaultDirection } = (this as any).getOptions(cfg) as NodeConfig;
    ...
    getPath(cfg: ModelConfig) {
      const { direction: defaultDirection } = (this as any).getOptions(cfg) as NodeConfig;
    ...
    getShapeStyle(cfg: NodeConfig) {
      const { style: defaultStyle } = (this as any).getOptions(cfg) as NodeConfig;
    ...
    update(cfg: NodeConfig, item: Item) {
      ...
      const { style: defaultStyle } = (this as any).getOptions({}) as NodeConfig;
      const path = (this as any).getPath(cfg);
    ...
    updateLinkPoints(cfg: NodeConfig, group: IGroup) {
      const { linkPoints: defaultLinkPoints, direction: defaultDirection } = (this as any).getOptions(
        {},
      ) as NodeConfig;
